...
It seems that the simpler model is a much worse model to predict pain in the patients. The simple model explains 7,5% of variance while the more complex model (after dropping a variable due to multicollinearity reasons) accounts for 32%. 
## Read the data
...
library(broom)
library(olsrr)
toothache_full <- read_excel("data/assignment_3_dataset.xlsx")
```

## Data and model diagnostics 
### Data diagnostics
It seems that there is one incorrect value (out of range) in both pain and mindfulness. The pain problem seems to be a typo (50 instead of 5) and I will treat it as that, the mindfulness is hard to guess, I will treat is as a maximum value (6). There was also an error in the variable "sex" with one entry of "woman" instead of "female"
...
```{r}
skim(toothache_full)
ggplot(toothache_full, aes(x=STAI_trait)) + geom_histogram()
ggplot(toothache_full, aes(x=pain_cat)) + geom_histogram()
ggplot(toothache_full, aes(x=mindfulness)) + geom_histogram()
ggplot(toothache_full, aes(x=pain)) + geom_histogram()
toothache_full %>%
   count(pain)
toothache_full %>% 
...
```{r}
toothache_full <- toothache_full %>%
     mutate(pain = str_replace(pain, "50", "5")) %>%
     mutate(mindfulness = str_replace(mindfulness, "6.22", "6")) %>%
     mutate(sex = str_replace_all(sex, "female", "2")) %>%
     mutate(sex = str_replace_all(sex, "male", "0")) %>%
     mutate(sex = str_replace_all(sex, "woman", "1"))
toothache_full<- toothache_full %>%
   mutate(pain = as.numeric(pain)) %>%
   mutate(sex = as.numeric(sex)) %>%
   mutate(mindfulness = as.numeric(mindfulness))
...
```{r}
complex_model <- lm(pain ~ age + sex + STAI_trait + pain_cat + mindfulness + cortisol_serum + cortisol_saliva, data = toothache_full) 
```
...
```{r}
updt_complex_model <- lm(pain ~ age + sex + STAI_trait + pain_cat + mindfulness + cortisol_serum, data = toothache_full)
```
...
```{r}
plot(updt_complex_model, 2)
```

Linearity assumption

```{r}
plot(updt_complex_model, 1)
```

Homoscedasticty assumption (homogeneity of variance)

```{r}
plot(updt_complex_model, 3)
```

Multicollinearity assumption

```{r}
ols_coll_diag(updt_complex_model)
...
```{r}
simple_model <- lm(pain~age + sex, data = toothache_full)
